...
// Calculates the distance between two points in cartesian system.
...
// Calculates next waypoint infront of the car
...
int main() {
  ...
  // Start in lane 1
  int lane = 1;
  // Iteration count used later
  int iteration = 0;
  
  // Have a reference velocity to target
  double ref_vel = 0; //mph

  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy, &lane, &ref_vel, &ptg, &iteration](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            bool too_close = false; // Variable determines if the car is too close to a car in front.
            bool way_too_close = false; // Additional variable for extra breaking when necessary.
            bool switch_lane = false; // Variable determines if lane should be switched, if there is a car in front in same lane.
          
            vector<vector<double>> cars_in_front; // Vector where all cars in front will be saved for the  optimal lane determination.
            vector<vector<double>> cars_in_range; // Vector where all cars in specific range will be saved for the optimal lane determination.
